...
const EditRoom: React.FC<PostRoomProps> = ({ rooms, roomIndex, setEditForm }) => {
        ...
        if (selectedFiles) {
            const validFiles = Array.from(selectedFiles).filter((file) => {
                if (file.size > maxSizeInBytes) {
                    console.warn(`${file.name} vượt quá giới hạn kích thước ${maxSizeInMB} MB`);
                    return false;
                }
                return true;
            });
        ...
        try {
            ...
            const uploadImages = await Promise.all(
                validFiles.map(async (file) => {
                    const storageRef = ref(storage, `${urlSaveImages}/${file.name}`);
                    try {
            ...
            const successfulUploads = uploadImages.filter((result) => result !== null);
        ...
        <div className="fixed flex items-center justify-center top-0 bottom-0 left-0 right-0 z-[9999]">
            ...
            >
                <div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="mt-3 roboto-regular">
                        <div>
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-3">
                            <div className="col-span-1 grid grid-cols-4 gap-1">
                                ...
                                <input
                                    type="text"
                                    {...register('contactNumber', { required: true, minLength: 9 })}
                                    className="col-span-3 px-2 py-1 rounded-[10px] border-[1px] outline-none w-full"
                                />
                            </div>
                            <div className="col-span-1 grid grid-cols-4 gap-1">
                                <label className="roboto-bold col-span-1">Email liên hệ:</label>
                                <input
                                    type="email"
                        ...
                        <div className="mt-3 flex items-center">
                            ...
                            <div className="flex items-center ml-3">
                                ...
                                <input
                                    type="number"
                                    {...register('acreage', { required: true, min: 1 })}
                                    className="ml-1 mt-1 border-2 outline-none rounded-[10px] px-2 py-1"
                                />
                            </div>
                        </div>
                        ...
                        <div className="mt-3">
                            ...
                            <p className="ml-2 text-[#ccc] text-[0.9rem]">
                                (Phải bao gồm hình ảnh mặt tiền, các phòng, nhà vệ sinh, chỗ nấu ăn nếu có, đường ngõ liền kề)
                            </p>
                            <div className="flex items-center">
                                        ...
                                        <div className="col-span-1 relative" key={index}>
                                            {changeImage[index] ? (
                                                <div className="flex flex-col items-center justify-center">
                                                    ...
                                                    <p className="text-center">{captions[index]}</p>
                                                </div>
                                            ) : (
                                                <div className="flex flex-col items-center justify-center">
                                                    <div
                                                        onClick={() => {
                                                            if (uploadImage.current[index]) {
                                                                uploadImage.current[index].click();
                                                            }
                                                        }}
                                            ...
                                            <input
                                                key={index}
                                                type="file"
                                                ref={(el) => (uploadImage.current[index] = el)}
                        ...
                        <div className="mt-3 roboto-bold">
                            ...
                            <div className="mt-1 w-full flex items-center">
                                <input
                                    ...
                                    type="text"
                                    className="rounded-[10px] px-2 py-1 border-2 outline-none w-full"
                                />
                                {/* Nút Tìm kiếm đã được xoá */}
                            </div>
